...
export function feederQualifiers(cls, take = "top2") {
  ...
  const topBy = (key) => active
    .filter((e) => e[key] != null && e[key] !== -1) // -1 = DQ, never qualifies
...
// Championship titles are PER JUDGE (judges are never combined): each judge's
// 1st is a Champion and their 2nd a Reserve, so a two-judge class can have
// two Champions. Returns Sets of entry ids; a horse that is one judge's
// Champion and the other's Reserve reads as Champion.
export function championshipTitles(cls) {
  const active = (cls?.entries ?? []).filter((e) => !e.scratched);
  const placing = isPlacingMode(cls);
  const judges = ["score", ...(cls?.judge2 ? ["score2"] : [])];
  const champions = new Set();
  const reserves = new Set();
  judges.forEach((key) => {
    const sorted = active
      .filter((e) => e[key] != null && e[key] !== -1) // -1 = DQ
      .sort((a, b) => (placing ? a[key] - b[key] : b[key] - a[key]));
    if (sorted[0]) champions.add(sorted[0].id);
    if (sorted[1]) reserves.add(sorted[1].id);
  });
  champions.forEach((id) => reserves.delete(id));
  return { champions, reserves };
}
