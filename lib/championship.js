...
export const looksLikeChampionship = (name) => /champ/i.test(String(name ?? ""));
...
// CHAMPION" instead collects the championship classes above it until the
...
export function suggestFeederIds(champLike, classes) {
  ...
  const grand = looksLikeGrand(champLike.name);
  const out = [];
  for (let i = before.length - 1; i >= 0; i--) {
    const c = before[i];
    if (grand) {
      if (looksLikeGrand(c.name)) break;
